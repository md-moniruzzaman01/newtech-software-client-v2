...
const RepairApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getAllProductsForRepair: builder.query({
      query: (params) => {
        return {
          url: `/repair?warranty=true&${params?.query}`,
          headers: {
            authorization: params?.token,
          },
        };
    ...
    getAllRepairService: builder.query({
      query: ({ token, query }) => {
        return {
          url: `/repair?warranty=false&${query}`,
          headers: {
            authorization: token,
          },
        };
    ...
    getRepairsForRequested: builder.query({
      query: (params) => {
        return {
          url: `/repair/my-library/${params?.id}?warranty=true&status=Required%20Parts&${params?.query}`,
          headers: {
            authorization: params?.token,
          },
        };
    ...
    getRepairsForRequestedForService: builder.query({
      query: (params) => {
        return {
          url: `/repair/my-library/${params?.id}?warranty=false&status=Required%20Parts&${params?.query}`,
          headers: {
            authorization: params?.token,
          },
        };
